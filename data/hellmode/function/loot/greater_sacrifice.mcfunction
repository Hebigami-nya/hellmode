...
execute at @e[tag=altar_greater_sacrifice] run kill @e[type=item,distance=..5,nbt={Item:{components:{"minecraft:custom_data":{isGreaterSacrifice:1b}}}}]
execute at @e[tag=altar_greater_sacrifice] run particle explosion_emitter ~ ~1.15 ~ 0.1 0.1 0.1 0.0001 20 normal
execute at @e[tag=altar_greater_sacrifice] run playsound minecraft:entity.ender_dragon.death ambient @a[distance=..50]
